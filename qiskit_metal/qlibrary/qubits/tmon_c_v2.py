...
class Tmon_c(BaseQubit):
    """The base `Tmon_c` class.
    ...
    Has only one connector line and it is added by using the `flux-bias line` dictionaries.
    The connector pad has a name and a list of default properties.
    ...
    Default Options:
        ...
        * pad_armgap: '10um' -- The distance around the arms to the ground, arms will be in FX pocket.
        ...
        * flux_bias_line_options=Dict
            ...
            * fbl_length: '30um' --  The width of the flux bias line along the x-axis
    ...
    def make_pocket(self):
        ...
        JJpad_bottom = draw.Polygon([
             ...
             (pad_head_width/3., pad_head_length+jj_gap/1.5),   # point c
             (pad_head_width/4., pad_head_length+jj_gap/1.5),   # point d
             ...
             (-pad_head_width/4., pad_head_length+jj_gap/1.5),  # point h
             (-pad_head_width/3., pad_head_length+jj_gap/1.5),  # point i
        ])

        JJpad_up_left = draw.Polygon([
             (-pad_head_width/3.+jj_width, pad_head_length+jj_gap),   # point k
             (-pad_head_width/4.+jj_width, pad_head_length+jj_gap),    # point l
             (-pad_head_width/4.+jj_width, pad_head_length+jj_gap/1.5),   # point m
             (-pad_head_width/3.+jj_width, pad_head_length+jj_gap/1.5),   # point n
        ])        

        JJpad_up_rigth = draw.Polygon([
             (pad_head_width/4.+jj_width, pad_head_length+jj_gap),  # point o
             (pad_head_width/3.+jj_width, pad_head_length+jj_gap),  # point p
             (pad_head_width/3.+jj_width, pad_head_length+jj_gap/1.5),   # point r
             (pad_head_width/4.+jj_width, pad_head_length+jj_gap/1.5),  # point s
        ])  

        JJpad_up=draw.union(JJpad_up_left, JJpad_up_rigth)
   ...
   #     jj_o = float(p.jj_orientation) # one can change the JJ orientation. Fab related detail.
      #  rect_jj = draw.rectangle(jj_width, jj_width/2, -pad_width/16, pad_gap/12)
        rect_jj1 = draw.LineString([((-pad_head_width/4.-pad_head_width/4.+jj_width)/2., pad_head_length+jj_gap/1.5), 
                                    ((-pad_head_width/4.-pad_head_width/4.+jj_width)/2., pad_head_length+jj_gap/2.)])
        rect_jj2 = draw.LineString([((pad_head_width/3.+pad_head_width/3.+jj_width)/2, pad_head_length+jj_gap/1.5), 
        ...
        pad_palm_left = draw.Point(-pad_arm_length/2-palm_radius, 0).buffer(palm_radius+circle_gap) 
        sub_left=draw.Point(-pad_arm_length/2-palm_radius, 0).buffer(palm_radius)
        cut_sq_l = draw.box(-palm_radius-pad_arm_length/2-circle_gap-palm_radius, -palm_radius-circle_gap, x_gap-pad_arm_length/2-palm_radius, palm_radius+circle_gap)
        pad_palm_left=draw.subtract(pad_palm_left,cut_sq_l)
        pad_palm_left=draw.subtract(pad_palm_left,sub_left)
        pad_palm_right = draw.Point(pad_arm_length/2+palm_radius, 0).buffer(palm_radius+circle_gap)
        sub_right=draw.Point(pad_arm_length/2+palm_radius, 0).buffer(palm_radius)
        cut_sq_r = draw.box(-x_gap+pad_arm_length/2+palm_radius, -palm_radius-circle_gap, palm_radius+pad_arm_length/2+circle_gap+palm_radius, palm_radius+circle_gap)
        ...
        pad_palm_gap_left = draw.Point(-pad_arm_length/2-palm_radius, 0).buffer(palm_radius+circle_gap+r_gap)
        sub_gap_left=draw.Point(-pad_arm_length/2-palm_radius, 0).buffer(palm_radius-r_gap)
        cut_gap_sq_l = draw.box(-palm_radius-pad_arm_length/2-circle_gap-palm_radius-r_gap, -palm_radius-circle_gap-r_gap, x_gap-pad_arm_length/2-palm_radius-r_gap, palm_radius+circle_gap+r_gap)
        pad_palm_gap_left=draw.subtract(pad_palm_gap_left,cut_gap_sq_l)
        pad_palm_gap_left=draw.subtract(pad_palm_gap_left,sub_gap_left)
        pad_palm_gap_right = draw.Point(pad_arm_length/2+palm_radius, 0).buffer(palm_radius+circle_gap+r_gap)
        sub_gap_right=draw.Point(pad_arm_length/2+palm_radius, 0).buffer(palm_radius-r_gap)
        cut_gap_sq_r = draw.box(-x_gap+pad_arm_length/2+palm_radius+r_gap, -palm_radius-circle_gap-r_gap, palm_radius+pad_arm_length/2+circle_gap+palm_radius+r_gap, palm_radius+circle_gap+r_gap)
        pad_palm_gap_right=draw.subtract(pad_palm_gap_right,cut_gap_sq_r)
        pad_palm_gap_right=draw.subtract(pad_palm_gap_right,sub_gap_right)
    ...
    def make_flux_bias_line(self):
        ...
        flux_bias_line = draw.Polygon([
             (-fbl_length/2-cpw_width, d+fbl_height+fbl_sep),   # point a
             (-fbl_length/2, d+fbl_height+fbl_sep),    # point b
             (-fbl_length/2, d+fbl_sep+cpw_width),   # point c
             (fbl_length/2, d+fbl_sep+cpw_width),   # point d
             (fbl_length/2, d+2*(fbl_height+fbl_sep)),   # point e
             (fbl_length/2+cpw_width, d+2*(fbl_height+fbl_sep)),   # point f
             (fbl_length/2+cpw_width, d+fbl_sep),   # point g
             (-fbl_length/2-cpw_width, d+fbl_sep),   # point i
        ])
         
        # Flux Bias line's gap part
        flux_bias_line_gap = draw.Polygon([
             (-fbl_length/2-cpw_width-cpw_gap, d+fbl_height+fbl_sep),   # point a
             (-fbl_length/2+cpw_gap, d+fbl_height+fbl_sep),    # point b
             (-fbl_length/2+cpw_gap, d+fbl_sep+cpw_width+cpw_gap),   # point c
             (fbl_length/2-cpw_gap, d+fbl_sep+cpw_width+cpw_gap),   # point d
             (fbl_length/2-cpw_gap, d+2*(fbl_height+fbl_sep)),   # point e
             (fbl_length/2+cpw_width+cpw_gap, d+2*(fbl_height+fbl_sep)),   # point f
             (fbl_length/2+cpw_width+cpw_gap, d+fbl_sep-cpw_gap),   # point g
             (-fbl_length/2-cpw_width-cpw_gap, d+fbl_sep-cpw_gap),   # point i
        ])
        
        # Flux-Bias Line CPW wire
        port_line = draw.LineString([(0, d+2*(fbl_height+fbl_sep)), 
                                    ((fbl_length+cpw_width), d+2*(fbl_height+fbl_sep))]) #point e
