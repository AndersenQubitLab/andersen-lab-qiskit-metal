...
class FluxoniumPocket(BaseQubit):
    ...
    Same fluxonium pocket but int his class JJ pads are included. 
    ...
    Default Options:
        ...
        * jj_length='10um' -- TODO: explain
        * jj_orientation: '1' -- Direction of the main JJ, fab related-all the JJs has to look at the same direction (Manhattan style JJ)
        * pad_width: '15um' -- The width (x-axis) of the charge island pads
        * pad_height: '100um' -- The size (y-axis) of the charge island pads
        * pad_radius: '60um' -- The radius of the circles at the end of the pads
        * inductor_orientation: '-1' -- Direction of the main JJ, fab related-all the JJs has to look at the same direction (Manhattan style JJ)
        * l_width: '1um' -- The width of the kinetic inductor along the x-axis
        * array_length: '130um' -- The length of the array along the y-axis
        * l_arm_width: '2um' -- The width of the arm of the kinetic inductor along the y-axis
        * l_arm_length: '20um' -- The length of the arm of the kinetic inductor along the x-axis
        * l_inductance: '200nH' -- Inductance value. One can calculate the inductance energy by that
        * L_j: '34.38nH' -- E_J can be calculated by it.
    ...
    def make_pocket(self):
        ...
        JJpad_up = draw.Polygon([
             (pad_width/16, 0),   # point g
             (pad_width/6, 0),    # point h
             (pad_width/6, pad_gap/2),   # point i
             (-pad_width/6, pad_gap/2),   # point k
             (-pad_width/6, pad_gap/3),  # point l
             (pad_width/16, pad_gap/6),  # point m
        ])        
        JJpad_bottom = draw.Polygon([
             (-pad_width/6, 0),   # point a
             (-pad_width/6, -pad_gap/2),    # point b
             (pad_width/6, -pad_gap/2),   # point c
             (pad_width/6, -pad_gap/3),   # point d
             (-pad_width/16, -pad_gap/6),  # point e
             (-pad_width/16, 0),  # point f
        ])
      ...
      #  rect_jj = draw.rectangle(jj_width, jj_width/2, -pad_width/16, pad_gap/12)
        rect_jj = draw.LineString([(-pad_width/16*jj_o, -jj_length/10*jj_o), 
                                   (jj_width*jj_o, -jj_length/10*jj_o)])
        ...
        pad_top = draw.union(pad_rect_top, pad_circle_top, l_arm_up, l_arm_up_fillet, JJpad_up)
        ...
        pad_bot = draw.union(pad_rect_bot, pad_circle_bot, l_arm_bot, l_arm_bot_fillet, JJpad_bottom)
    ...
    def make_flux_bias_line(self):
        ...
        # Here, we make the flux-bias line curvy for the top side and also the bottom side and then union all of them
